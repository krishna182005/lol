    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: product?.name,
          text: `Check out this ${product?.category} from TrustyLads`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <nav className="text-sm text-gray-500">
            <span>Home</span> / <span>Shop</span> / <span className="capitalize">{product.category}</span> / <span className="text-gray-900">{product.name}</span>
          </nav>
        </motion.div>
          ...
          >
            <div className="sticky top-8">
              ...
              <div className="relative mb-4 bg-white rounded-lg overflow-hidden shadow-md">
                <img
                  src={product.images[selectedImageIndex]?.url || '/api/placeholder/600/800'}
                  alt={product.name}
